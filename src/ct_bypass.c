...
char *extract_best_slice(const char *fat_path) {
    Fat *fat = fat_init_from_path(fat_path);
    if (!fat) {
        return NULL;
    }
...
int apply_ct_bypass(const char *macho_path) {
    MachO *macho = macho_init_for_writing(macho_path);
    if (!macho) {
        fprintf(stderr, "Failed to open MachO file '%s' for writing.", macho_path);
        return -1;
    }
